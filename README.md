...
Runs on: http://localhost:8080/api/posts
...
```
1. Application Process (Server)
  cd microservices/post-service/post-service-application
  mvn spring-boot:run
```
...
```
  java -jar microservices/post-service/target/post-service-1.0.0.jar
```
...
* Call this get request a few times, each call will create a new post with a 'pre_calc' state, which will be updated by a scheduler service (AlgoService):
